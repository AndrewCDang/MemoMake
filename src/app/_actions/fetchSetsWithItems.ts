...
const fetchSetsWithItems = async({setIds}:{setIds:string[]}): Promise<Flashcard_set_with_items[]    => {
    try {
        const setWithItems:Flashcard_set_with_items[] = db`
        SELECT fs.*, array_agg(to_json(fi))) AS flashcards FROM flashcard_set fs
        LEFT JOIN flashcard_item fi ON fs.id = ANY(fi.set_id)
